...
def get_luna_pretrain_list(ratio):
    ...
    """
    x_train = []
    with open("net/pretrain/PCRLv2/train_val_txt/pretrain_train.txt", "r") as f:
        for line in f:
...
def get_luna_list(config, train_fold, valid_fold, test_fold, suffix, file_list):
    ...
    Args:
        config (_type_): arg
        train_fold (list[int]): _description_
        valid_fold (_type_): _description_
        test_fold (_type_): _description_
        suffix (str): 通常是 __global__
        file_list (_type_): 给定的训练数据名称列表
    ...
    x_valid = []
    x_test = []
    # for i in train_fold:  # 首先对位于 train_fold 中的数据检查
    ...
    for file in os.listdir(config.data):
        if suffix in file:
            if file.split(suffix)[0] in file_list:
                x_train.append(os.path.join(config.data, file))
                # elif file_list is None:
            else:
                x_valid.append(os.path.join(config.data, file))
                x_test.append(os.path.join(config.data, file))
