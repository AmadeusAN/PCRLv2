...
from utils import get_file_name_without_extend_from_absolutepath

sys.setrecursionlimit(40000)

parser = OptionParser()
parser.add_option(
    "--fold", dest="fold", help="fold of subset", default=None, type="int"
)
parser.add_option(
    "--input_rows", dest="input_rows", help="input rows", default=64, type="int"
)
parser.add_option(
    "--input_cols", dest="input_cols", help="input cols", default=64, type="int"
)
parser.add_option(
    "--input_deps", dest="input_deps", help="input deps", default=32, type="int"
)
parser.add_option(
    "--crop_rows", dest="crop_rows", help="crop rows", default=64, type="int"
)
parser.add_option(
    "--crop_cols", dest="crop_cols", help="crop cols", default=64, type="int"
)
parser.add_option(
    "--data",
    dest="data",
    help="the directory of LUNA16 dataset",
    default="./data1/luchixiang/LUNA16",
    type="string",
)
parser.add_option(
    "--save",
    dest="save",
    help="the directory of processed 3D cubes",
    default="/public1/cjh/workspace/AbdominalSegmentation/dataset/PCRL_cache",
    type="string",
)
parser.add_option(
    "--scale", dest="scale", help="scale of the generator", default=16, type="int"
)
...
col_size = [(96, 96, 64), (96, 96, 96), (96, 96, 64), (64, 64, 32)]
...
def infinite_generator_from_one_volume(img_array, save_dir, name):
    ...
    while True:
        ...
        np.save(
            os.path.join(save_dir, name + "_global_" + str(num_pair) + ".npy"),
            crop_window,
        )
        np.save(
            os.path.join(save_dir, name + "_local_" + str(num_pair) + ".npy"),
            local_windows,
        )
        num_pair += 1
...
def crop_pair(img_array):
    while True:
        ...
        if (
            size_z - 64 - config.len_depth - 1 - config.len_border_z
            < config.len_border_z
        ):
            pad = (
                size_z
        ...
        while True:
            ...
            if size_x - crop_rows2 - 1 - config.len_border <= config.len_border:
                crop_rows2 -= 32
                crop_cols2 -= 32
            start_x1 = random.randint(
                0 + config.len_border, size_x - crop_rows1 - 1 - config.len_border
            )
            start_y1 = random.randint(
                0 + config.len_border, size_y - crop_cols1 - 1 - config.len_border
            )
            start_z1 = random.randint(
                0 + config.len_border_z,
                size_z - crop_deps1 - config.len_depth - 1 - config.len_border_z,
            )
            start_x2 = random.randint(
                0 + config.len_border, size_x - crop_rows2 - 1 - config.len_border
            )
            start_y2 = random.randint(
                0 + config.len_border, size_y - crop_cols2 - 1 - config.len_border
            )
            start_z2 = random.randint(
                0 + config.len_border_z,
        ...
        crop_window1 = img_array1[
            start_x1 : start_x1 + crop_rows1,
            start_y1 : start_y1 + crop_cols1,
            start_z1 : start_z1 + crop_deps1 + config.len_depth,
        ]

        crop_window2 = img_array2[
            start_x2 : start_x2 + crop_rows2,
            start_y2 : start_y2 + crop_cols2,
            start_z2 : start_z2 + crop_deps2 + config.len_depth,
        ]

        if (
            crop_rows1 != input_rows
            or crop_cols1 != input_cols
            or crop_deps1 != input_depth
        ):
            crop_window1 = resize(
                crop_window1,
        ...
        if (
            crop_rows2 != input_rows
            or crop_cols2 != input_cols
            or crop_deps2 != input_depth
        ):
            crop_window2 = resize(
                crop_window2,
                (input_rows, input_cols, input_depth + config.len_depth),
        ...
        for i in range(6):
            ...
            local_crop_rows, local_crop_cols, local_crop_deps = local_col_size[
                local_size_index
            ]
            local_window = img_array1[
                local_x : local_x + local_crop_rows,
        ...
        return (
            crop_window1[:, :, :input_depth],
            crop_window2[:, :, :input_depth],
            np.stack(local_windows, axis=0),
        )
...
def get_file_path_list():
    ...
    datalist = [item["image"] for item in train_list]
    return datalist
...
def get_self_learning_data():
    ...
    Args:
        fold (_type_): 仅是对 LUNA16 数据集的一个子集进行处理
    """
    ...
    # 对 file_list 中的每个图像都生成一些预处理的数据
    for i, img_file in enumerate(tqdm(file_list)):
        # img_name = os.path.split(img_file)[-1]
        img_name = get_file_name_without_extend_from_absolutepath(img_file)
        img_array = load_sitk_with_resample(img_file)
        img_array = sitk.GetArrayFromImage(img_array)
        img_array = img_array.transpose(2, 1, 0)
        # print(img_array.shape)
        infinite_generator_from_one_volume(img_array, save_dir, img_name)
